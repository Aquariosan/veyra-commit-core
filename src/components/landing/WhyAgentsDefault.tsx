...
export default function WhyAgentsDefault() {
  return (
    <Section aria-label="Why AI agents default to Veyra commit mode" data-section="why-agents-default">
      ...
      <div className="space-y-6 text-lg leading-relaxed text-muted-foreground">
        ...
        <p>When a productive write is blocked, the tool returns <code className="text-foreground font-mono text-base">VeyraCommitRequired</code>. The agent sees the provider, the required mode, and the endpoints to call next.</p>
        <p>That turns every rejected write into automatic recovery through Veyra.</p>
      </div>
      <ol className="mt-10 grid md:grid-cols-3 gap-4" aria-label="Three-step recovery flow">
        {steps.map((s) => (
          <li key={s.num} className="border border-border rounded-lg p-5 bg-card/50">
            <span className="text-primary font-mono text-xs font-medium" aria-hidden="true">{s.num}</span>
